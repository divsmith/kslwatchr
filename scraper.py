...
data = re.compile(detail_page_regex, flags).findall(get_listing_details) # match listing data
print(data)
if isinstance(data, list):
    # remove leading spaces and \n
    listing_data = re.sub(r'\s+',' ', data[0][0])
    seller_data = re.sub(r'\s+',' ', data[0][1])

    # convert to json object
    isting_data = demjson.decode(listing_data)
    seller_data = demjson.decode(seller_data)

    # pretty prints as array of json objects
    print(json.dumps(listing_data, sort_keys=True, indent=4))
    print(json.dumps(seller_data, sort_keys=True, indent=4))
else:
    print("listing data matched was not a list or tuple")